...
class Router extends EventEmitter {
  ...
  _onConnection (sock) {
    ...
    peer.on('join', () => {
      ...
      context.members.set(peer.info.name, info);
    });

    peer.on('logout', () => {
      peer.joined && io.error(`Peer ${peer.info.name} exited`);

      // remove peer info
      context.peers.delete(peer.info.name);
      context.members.delete(peer.info.name);
