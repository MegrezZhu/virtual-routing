...
class Router extends EventEmitter {
  ...
  async installAlgo (algo) {
    await algo.init(this);
    this.algo = algo;
  }
  ...
  _handleNodeEvent (node) {
    ...
    node.on(Message.MESSAGE, msg => {
      if (msg.header.to === this.name) {
        ...
        // forwarding
        const route = this.algo.getRoute().get(msg.header.to);
        if (!route || route.len === 16) return io.error(`Packet Forwarding (${msg.header.from} → ${msg.header.to}): Route to ${msg.header.to} not exists!`);
    ...
    node.once('close', this._onNeighborGone.bind(this, node));
  }

  _onNeighborGone (node) {
    this.neighbors.delete(node.info.name);
    io.result(`Lost connection with: ${node.info.name}`);
    this.emit(Router.NEIGHBOR_GONE, node);
  }

  send (name, data) {
    const route = this.algo.getRoute().get(name);
    assert(route && route.length !== 16, `Route to "${name}" not exists!`);
    const byName = route.by;
    assert(byName, `${chalk.red('[Fatal]')} Route to ${name} not exists!`);

    const node = this.neighbors.get(byName);
    node.send(new Message(Message.MESSAGE, name, {
      type: Router.ECHO,
      data
    }));
  }
...
Router.NEIGHBOR_GONE = 'ROUTER.NEIGHBOR_GONE';
